...
function Product() { 
  ...
  return (
    <div className='  mt-24 sm:pl-10 md:pl-44 flex flex-col sm:flex-row '>
      ...
      {
        price === listedPrice ? <p  className='font-semibold'>{" ₹ "+listedPrice}</p> :

        ( 
        <p className='font-semibold'>{" ₹" + price + "  "} 
        <span className="text-gray-500 ">{"MRP "}</span>
        <span className="text-gray-500 line-through"> {" ₹" + listedPrice}</span>
        <span className=' text-red-600'>{" "+ Math.ceil((listedPrice - price) / listedPrice * 100)}% Off</span></p>)
        }
      </div>
      <p className=' text-green-600 px-5 text-sm'>Inclusive of all taxes</p>
      <div className=' my-4 flex'>
        {
          !btnClicked ? <button className=' m-3 mx-4 sm:m-3 p-3 flex justify-center text-sm border border-black hover:shadow-lg hover:bg-gray-100' onClick={()=>dispatch(addToCart({...data}), setBtnClicked(true))}>ADD TO CART</button>
